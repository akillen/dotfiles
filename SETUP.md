...
- **Node Module**: [scripts/setup-node.sh](scripts/setup-node.sh) — Installs and configures Node via `asdf`, upgrades `npm`, and verifies `npx`.
...
- Node.js setup is delegated to `scripts/setup-node.sh` (`asdf`, latest `npm`, and `npx` availability check).
